...
local vel_Flag = true --Flags if a velocity test is being conducted
...
local vel_amp = 2 -- Amplitude of the Velocity multisine
local att_amp = 10 -- Amplitude of the Attitude multisine
...
--Setup
file_name = "/scripts/" .. file_name .. ".csv" -- Formats the code so that it can be found by the io.open function
...
function update()
  ...
  if pos_Flag then
    if not File then --Checks if the file is opened and if it isn't it attempts to open it
      ...
      trimmed_alt = -position_ned:z() --Save the base altitude
  ...
  elseif vel_Flag then
    ...
    elseif elapsed_time < csv_data[#csv_data].time then --If the code has reached this point then then it is ready to find the value to be written
      ...
      --gcs:send_text(6, tostring(delta_velocity_north)) commented to reduce bloat
