...
function update()
    ...
    if vehicle:get_mode() ~= 4 then -- 4 is GUIDED mode in Copter
    ...
    -- Calculate current target angles using sinusoidal functions (in DEGREES)
    ...
    -- Base climb rate to keep it airborne (0.0 = maintain current altitude hold)
    local climb_rate_ms = 0.0 
    
    -- Send target angles (ArduPilot expects DEGREES here) to the vehicle controller
    -- vehicle:set_target_angle_and_climbrate(roll_deg, pitch_deg, yaw_deg, climb_rate_ms, use_yaw_rate, yaw_rate_degs)
...
gcs:send_text(6, "Sinusoid Attitude Script Loaded")
return update()
